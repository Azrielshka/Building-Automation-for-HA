...
**Этапы 0–11 закрыты (тег `0.6.0`).** Реализовано и проверено на песочнице
HA 2026.5.4; приёмка — [`docs/acceptance-protocol.md`](docs/acceptance-protocol.md).
Этап 11 — управление автояркостью соседней интеграции из матрицы режимов +
индикация (колонка «Автояркость»); индикация **принята на объекте**.
Остаточные полевые пункты до `v1.0.0` — подтверждение видимого света и живого
управления автояркостью на объекте с DALI.
