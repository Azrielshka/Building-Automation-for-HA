...
[`docs/requirements/TZ.md`](../../docs/requirements/TZ.md) §2. Нарушение означает, что интеграция полезла в
чужую епархию.
...
перезагрузке подписчика (см. `docs/requirements/TZ.md` §5.1).
...
[`docs/requirements/TZ.md`](../../docs/requirements/TZ.md) §7.5; менять состав или имена ключей можно
...
[`docs/requirements/task.md`](../../docs/requirements/task.md) — вся логика живёт в Python, состояние в
`.storage`.
...
### `__init__.py` держит импорты HA отложенными

`custom_components/building_automation/__init__.py` неизбежно загружается при
импорте **любого** подмодуля `domain` (Python выполняет `__init__` пакета-родителя).
Пакет `homeassistant` в среде разработки не установлен (SPEC §2.1), поэтому если
`__init__` импортирует HA на уровне модуля — ломается импорт ядра, а с ним все
тесты.

Правило: в `__init__.py` импорты HA — **только под `TYPE_CHECKING`** (для
аннотаций) либо **отложенные внутри `async_setup_entry`** (для `Platform`,
`coordinator` и прочей обвязки). Это единственное оправданное место для импорта
внутри функции — помечать `# noqa: PLC0415` с этим обоснованием.

Остальные обвязочные модули (`coordinator`, `config_flow`, платформы, `adapters/`)
импортируют HA на уровне модуля свободно: ядро их не импортирует (сторож §5.7
это гарантирует), а грузятся они только в среде HA.

### Платформы сущностей — в корне пакета

Файлы платформ (`sensor.py`, `switch.py`, `select.py`, `binary_sensor.py`) лежат
в **корне** `custom_components/building_automation/`, не в подпапке. Home
Assistant ищет платформу по пути `<интеграция>/<платформа>.py`; подпапку loader
не сканирует.
...
- Покрытие `coordinator.py` — > 90 % (критерий из `docs/requirements/task.md`), считается по
  веткам.
